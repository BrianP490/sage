...
Description: Testing out Sagemaker training and inferrence.
"""
...
from src.weather_pred import get_uri, AgentInferenceSpec
...
env_vars = {
    # KNOWN ISSUES: MAY LEAD TO UNEXPECTED BEHAVIOR IF KEY IS POPULATED AND VALUES LEFT "" (EMPTY)
    "AWS_REGION": AWS_REGION,
    "AWS_BUCKET": AWS_BUCKET,
    "TRAIN_DATA_S3_URI": TRAIN_DATA_PATH,
    "TRAINING_JOB_NAME": JOB_NAME,
    "PYTHONPATH": "/opt/ml/input/data/code:/opt/ml/model/code",  # Resolves the importing config.json file issue
...
def main():
    ...
    """
    Cannot pass a Python boto3.Session object across the container boundary.
    The launcher and the training container are separate processes.
    You can pass environment variables (or mount files) that the container can use to construct its own session."""
    ...
    trainer = ModelTrainer(
        ...
        local_container_root=str(CONTAINER_ROOT),  # Ensures /opt/ml/model -> CONTAINER_ROOT / model
        # config_mgr=None
    )

    """
    local_container_root=CONTAINER_ROOT
    With this override, the SDK will bind-mount these paths:

    Host: <CONTAINER_ROOT>/model  ↔  Container: /opt/ml/model
    Host: <CONTAINER_ROOT>/output ↔  Container: /opt/ml/output
    Host: <CONTAINER_ROOT>/input  ↔  Container: /opt/ml/input
    (Plus a temp driver folder under <CONTAINER_ROOT>/<random>/ ↔ /opt/ml/input/data/sm_drivers)
    
    SM_MODEL_DIR → Where to save your model (collected as artifacts)
        Example: /opt/ml/model

    SM_OUTPUT_DIR → Base output dir; failure messages go to SM_OUTPUT_FAILURE
        Example: /opt/ml/output (failure file at /opt/ml/output/failure)

    SM_INPUT_DIR → Root input dir (configs + data)
        Example: /opt/ml/input

    SM_INPUT_DATA_DIR → Input data root
        Example: /opt/ml/input/data

    SM_INPUT_CONFIG_DIR → Where SageMaker writes config JSONs
        Example: /opt/ml/input/config

    SM_SOURCE_DIR → Your code directory inside container
        Example: /opt/ml/input/data/code

    SM_ENTRY_SCRIPT → The script to run (e.g., train.py)
    """

    # logger.info(f"Train Path: {TRAIN_DATA_PATH}")

    # Using LOCAL DATA
    # train_data = InputData(channel_name="training", data_source=TRAIN_DATA_PATH)    # Will be downloaded to `/opt/ml/input/data/<channel_name>`
    # trainer.train(input_data_config=[train_data])  # local Docker container
    ...
    # When you pass a tarball to ModelBuilder.model_path, the SDK unpacks it into the container’s /opt/ml/model (or equivalent staging dir) for you. You don’t need to manually copy individual files in prepare()
    ...
    builder = ModelBuilder(
        ...
        # },    # For deterministic dependencies
        env_vars={
            **env_vars,
        ...
        # accept_type="Optional",
        s3_model_data_url=FINAL_TARBALL_OUTPUT_S3_PATH,  # The S3/local location where you want to upload the model package,
        instance_type=INFER_INSTANCE_TYPE,
        schema_builder=SchemaBuilder(
            sample_input=SCHEMA_SAMPLE_INPUTS, sample_output=SCHEMA_SAMPLE_OUTPUTS
        ),  # Automatically detects the serializer and deserializer for your mode; Can create the translators using CustomPayloadTranslator class inherritance
        # model=None,  # Define object from which training artifacts can be extracted. Either model or inference_spec is required for the model builder to build the artifact.
        inference_spec=AgentInferenceSpec(),
        # inference_spec="The inference spec file with your customized invoke and load functions",
        # image_uri=INFER_IMAGE_URI,  # "Specify a specific container image",
        # image_config={
        #     "name": "None"
        # },
        # vpc_config={
        ...
        # },
        # model_server=ModelServer.TORCHSERVE,  # Should be specified when supplying image_uri ; Possible values for this argument are TORCHSERVE, MMS, TENSORFLOW_SERVING, DJL_SERVING, TRITON, TGI, and TEI
        # model_metadata={
        #     "CUSTOM_MODEL_PATH": "file:///mnt/c/Users/brianperez/Desktop/DEV/aws/Weather-Pred-AWS/local_mode/artifacts/model/model.pt"
        # },
        # inference_component_name="Optional",
        # modelbuilder_list=["Some components"],
        # resource_requirements=["Some requirements"],
    )
    ...
    try:
        ...
        endpoint = builder.deploy(
            ...
            # instance_type=infer_instance_type,  #  EC2 instance type to deploy this model to
            ...
            # inference_config=None,  # Pick deployment style (serverless, async, batch) or give resource hints
            # custom_orchestrator_instance_type=None,  # For advanced use
            # custom_orchestrator_initial_instance_count=None,  # For advanced use
        ...
        logger.error(f"❌ Pipeline failed: {str(e)}", exc_info=True, stack_info=True)
    finally:
        if endpoint:
