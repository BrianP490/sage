...
Description: Testing out Sagemaker training and inferrence on cloud.
"""
...
RUN_LOCAL = False  # Toggle this to False for managed SageMaker Operations
...
FINAL_TARBALL_OUTPUT_S3_PATH = f"s3://alexandria-reborn/CA-Weather-Fire/Compressed_Artifacts/"  # Where ModelBuilder will upload the packaged model
...
env_vars = {
    ...
    "PYTHONPATH": "/opt/ml/input/data/code:/opt/ml/model/code",  # Resolves the importing  issues
    "SAVE_MODEL": "true",
...
def main():
    ...
    src = SourceCode(
        source_dir=str(SOURCE_DIR),  # Everything inside the `source_dir` will be copied to `/opt/ml/code/**`
    ...
    try:
        ...
        # Returns a sagemaker.core.resources.Endpoint
