...
Description: Testing out Sagemaker training and inferrence locally.
"""
...
env_vars = {
    ...
    # "FEATURE_SCALER_URI": "",  # Bucket-relative!!
    # "LABEL_SCALER_URI": "",
...
if RUN_LOCAL:
    ...
    TMPDIR = str(Path.home()) + "/local_mode/tmp"
...
def main():
    ...
    # Need for the Feature/Label Scalers to be available inside the container
    # build_src = SourceCode(
    #     source_dir=str(SOURCE_DIR),  # Everything inside the `source_dir` will be copied to `/opt/ml/model/code/**`
    #     # entry_script="weather_pred/agent_inference/agent_inference_spec.py",  # Relative to the source_dir; Uses the default entry point/bootloader ("inference.py")
    #     # command="python train.py epochs=1",
    #     ignore_patterns=[
    #         ".env",
    #         ".git",
    ...
    #
    # In-Process	~/local_mode/model	Runs in your local Python env (no Docker).
    # Local Container	/tmp/models/.../model	Runs in Docker; mounts your local files.
    # Managed (Cloud)	/opt/ml/model	Runs in AWS; downloads from S3.

    # Preparation: Your prepare script copies your local WSL src code and model weights into a specific folder on your host: /tmp/models/UUID/model.
    # Mounting: When the container starts, SageMaker runs a Docker command similar to: docker -v /tmp/models/UUID/model:/opt/ml/model
    # Execution: Inside the container, any file you put in /tmp/models/UUID/model/code on the host appears magically at /opt/ml/model/code.
    builder = ModelBuilder(
        mode=SERVE_MODE,
        model_path=model_artifacts_locator,  # Local path or S3 URI to trained model artifacts (.tar.gz or directory)
        ...
        env_vars={
            **env_vars,
            ...
            # "SAGEMAKER_PROGRAM": "inference.py",  # If it's set, the toolkit adds /opt/ml/model/code to the Python path and attempts to import the filename specified.
        },
    ...
    # When ModelBuilder builds your model for TORCHSERVE, it creates a model.tar.gz (or a directory equivalent). Inside that package, it generates a inference.py.

    # If the environment variable SAGEMAKER_PROGRAM is not explicitly pointing to the generated wrapper that loads your serve.pkl (which contains your AgentInferenceSpec), TorchServe falls back to the Base Handler. The Base Handler is hardcoded to look for a standard forward() method or a predict_fn in a file named inference.py. Since it doesn't find those, it just performs a default identity transform on your TorchScript model.
    ...
    # When you call builder.build(), SageMaker uses a library called cloudpickle to package your AgentInferenceSpec into that serve.pkl file you saw in the /code directory.

    # Because your AgentInferenceSpec is defined inside weather_pred.py, the pickled object contains a reference that says: "I am an instance of the class AgentInferenceSpec located in a module named weather_pred."

    # When the container starts:
    # It runs the generated inference.py.
    # inference.py tries to unpickle serve.pkl.
    # Python looks for a file named weather_pred.py to reconstruct the class.
    ...
    try:
        ...
        # payload = json.dumps({"inputs": inputs})  # Convert to json string
        # payload = {"inputs": inputs}
        payload = json.dumps(inputs)
        try:
            # Sagemaker is able to dictate what to do to serialize the payload
